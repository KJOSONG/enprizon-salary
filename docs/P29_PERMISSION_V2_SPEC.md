...
> **状态**：✅ 方案已获用户批准（2026-08-23）｜⛔ **尚未开工，本文件是唯一权威需求来源**
...
## 13. 开放问题（不阻塞开工，实施中顺手确认）

1. `admin_users` 是否已有显示名列？（决定 §8 是否需要加列；没有则先显示用户名）
2. `/export/attendance` 双路由定义（3487/3819）哪个是死代码，合并时确认调用方。
3. applicant「只看自己的申请」在 OA 历史接口的过滤字段直接用 `operator_id==username` 即可，但需确认撤销(revoked)状态事件是否也对自己可见（建议可见）。
